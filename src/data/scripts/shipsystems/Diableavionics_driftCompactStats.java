...
public class Diableavionics_driftCompactStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, ShipSystemStatsScript.State state, float effectLevel) {
        ...
        if (ship != null) {
            ship.setJitterUnder(ship, Color.ORANGE, 0.5F * effect, 5, 5.0F + 5.0F * effect, 5.0F + 10.0F * effect);
            if (Math.random() > 0.8999999761581421D)
                ship.addAfterimage(new Color(255, 215, 0, 64), 0.0F, 0.0F, -(ship.getVelocity()).x, -(ship.getVelocity()).y, 5.0F + 50.0F * effect, 0.0F, 0.0F, 2.0F * effect, false, false, false);
    ...
    public ShipSystemStatsScript.StatusData getStatusData(int index, ShipSystemStatsScript.State state, float effectLevel) {
        if (index == 0)
            return new ShipSystemStatsScript.StatusData(this.TXT, false);
        return null;
    }
}
